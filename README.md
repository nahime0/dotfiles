...
create, rename, or delete tmux sessions. It uses the popup built into
...
The Omarchy profile also links `bin/tailscale-config` into
`~/.local/bin/tailscale-config`. It manages Tailscale exit nodes through an
interactive menu or CLI commands and requires `tailscale` and Python 3; `gum`
is optional for the menu. Run `tailscale-config --help` for available commands.
